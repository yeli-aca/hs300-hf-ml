...
# ============== PCA 降维 → DNN ==============
N_COMPONENTS = 5  # 可调，建议 5~10，看累计方差解释率
...
print(f"✅ DNN + PCA({N_COMPONENTS}) R²: {r2:.6f}")
print(f"📉 DNN + PCA({N_COMPONENTS}) MSE: {mse:.6e}")
...
y_pred_dnn = dnn.predict(Z_test).flatten()
report_result(f"DNN + PCA({N_COMPONENTS})", y_test, y_pred_dnn)
