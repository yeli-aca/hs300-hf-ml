...
def build_ipca_data(df, factor_cols_z):
    ...
    X = np.full((T, N, K), np.nan)

    for _, row in df.iterrows():
        t = date_index[row["datetime"]]
...
def run_ipca(Y, X, dates, codes, fac_names, out_dir, n_factors=3):
    """
    调用 C1 的 IPCA 类进行建模，并输出 Gamma 和因子。
    """
    from ipca_core import InstrumentedPCA  # 如果写在本文件则不用加

    T, N, K = Y.shape[0], Y.shape[1], X.shape[2]

    # 将 panel 数据转换为 flat 数据（和 C1 兼容）
    records = []
    for t in range(T):
        for n in range(N):
            if np.isnan(Y[t, n]):
            ...
            for k in range(K):
                row[fac_names[k]] = X[t, n, k]
            records.append(row)

    df_ipca = pd.DataFrame(records)
    df_ipca = df_ipca.dropna()
    df_ipca = df_ipca.reset_index(drop=True)
    
    # 构造 MultiIndex 和特征、目标
    ...
    print("✅ IPCA 运行完成")
    print(f"Gamma shape: {Gamma.shape}, Factors shape: {Factors.shape}")

    return model, Gamma, Factors
...
# ✅ 检查是否缺失必要列
    required = ["datetime", "code", "excess_ret_t1"] + use_cols
    missing = [c for c in required if c not in data.columns]
    if missing:
        raise ValueError(f"❌ IPCA 所需列缺失：{missing}")

        # IPCA 格式准备（你已有）
    Y_ipca, X_ipca, dates_ipca, codes_ipca, fac_names_ipca = build_ipca_data(data, use_cols)

    # IPCA 运行
    print(">> Running IPCA (approximation via PCA + regression) ...")
    model_ipca, Gamma, Factors = run_ipca(
        Y=Y_ipca, X=X_ipca, dates=dates_ipca, codes=codes_ipca,
        fac_names=fac_names_ipca,
        out_dir=CONFIG["OUT_DIR"],
        n_factors=3
    )

    res_ipca = run_ipca(Y_ipca, X_ipca, CONFIG["OUT_DIR"], fac_names_ipca)
    ...
    summary = {
        ...
        "ipca": {"R2": float(res_ipca["r2"]), "MSE": float(res_ipca["mse"])},  # ✅新增
        "cutoff": str(cutoff),
...
这是加入了ipca的代码，还在跑，是我对5min数据的所有程序与工作，分析整体流程以及完成了什么工作