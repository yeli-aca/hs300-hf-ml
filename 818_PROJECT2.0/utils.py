...
def build_features(panel: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    factor_cols = ["ret_5","ret_10","reversal_1","vol_5","vol_10","hl_range",
...
def industry_neutralize(df: pd.DataFrame, factor_cols: List[str], industry_col="industry"):
...
def plot_distribution(y: pd.Series, out_dir: str):
    ...
    plt.figure(figsize=(8,5)); plt.hist(y.dropna(), bins=120, alpha=0.85)
    plt.title("Excess Return (t+1) Distribution"); plt.xlabel("Excess Return (t+1)"); plt.ylabel("Count")
...
def plot_xgb_importance(model, feature_names, out_dir: str):
    ...
    if imp.isnull().all(): return
    imp = imp.sort_values()
    plt.figure(figsize=(8,6)); imp.plot(kind="barh")
    plt.title("XGBoost Feature Importance"); plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fig3_xgb_importance.png"), dpi=300); plt.close()
...
def plot_corr_heatmap(df: pd.DataFrame, feature_names: List[str], out_dir: str):
    ...
    corr = samp.corr()
    plt.figure(figsize=(8,6)); plt.imshow(corr, cmap="coolwarm"); plt.colorbar()
    plt.xticks(range(len(feature_names)), feature_names, rotation=90); plt.yticks(range(len(feature_names)), feature_names)
    plt.title("Correlation Heatmap"); plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fig4_corr_heatmap.png"), dpi=300); plt.close()
...
def plot_model_comparison(summary_json_path, out_dir):
    ...
    if not models: 
        print("[plot_model_comparison] no metrics"); return

    plt.figure(figsize=(6,4)); plt.bar(models, r2s); plt.title("Model Comparison: R²"); plt.ylabel("R²")
    ...
    plt.figure(figsize=(6,4)); plt.bar(models, mses); plt.title("Model Comparison: MSE"); plt.ylabel("MSE")
...
def plot_ic(ic_series, out_dir):
    if not CONFIG["PLOT"]: return
    plt.figure(figsize=(10,3.5)); plt.plot(ic_series.index, ic_series.values); plt.axhline(0, ls="--")
    plt.title("Cross-sectional Spearman IC (per bar)"); plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fig_ic_timeseries.png"), dpi=300); plt.close()
    plt.figure(figsize=(6,4)); plt.hist(ic_series.values, bins=50); plt.title("IC Distribution"); plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "fig_ic_hist.png"), dpi=300); plt.close()
...
def plot_quantiles(grp, cum, out_dir):
    ...
    plt.title("Quantile Portfolio Returns (per period)"); plt.legend(ncol=5, fontsize=8)
    plt.tight_layout(); plt.savefig(os.path.join(out_dir, "fig_quantile_returns.png"), dpi=300); plt.close()
    plt.figure(figsize=(9,4)); plt.plot(cum.index, cum["LS"])
    plt.title("Long-Short (Q5-Q1) Cumulative Return (no cost)")
    plt.tight_layout(); plt.savefig(os.path.join(out_dir, "fig_ls_cum.png"), dpi=300); plt.close()
